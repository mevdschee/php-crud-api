    ...
    class GenericReflection
    {
        ...
        private function getTablesSQL(): string
        {
            switch ($this->driver) {
                ...
                case 'pgsql':
                    return 'SELECT c.relname as "TABLE_NAME", c.relkind as "TABLE_TYPE" FROM pg_catalog.pg_class c WHERE c.relkind IN (\'r\', \'v\') AND c.relnamespace::regnamespace::text = \'public\' AND \'\' <> ? ORDER BY "TABLE_NAME";';
                case 'sqlsrv':
        ...
        private function getTablePrimaryKeysSQL(): string
        {
            switch ($this->driver) {
                ...
                case 'pgsql':
                    return 'SELECT a.attname AS "COLUMN_NAME" FROM pg_attribute a JOIN pg_constraint c ON (c.conrelid, c.conkey[1]) = (a.attrelid, a.attnum) JOIN pg_class pgc ON pgc.oid = a.attrelid WHERE pgc.relname = ? AND \'\' <> ? AND pgc.relnamespace::regnamespace::text = \'public\' AND c.contype = \'p\'';
                case 'sqlsrv':
        ...
        private function getTableForeignKeysSQL(): string
        {
            switch ($this->driver) {
                ...
                case 'pgsql':
                    return 'SELECT a.attname AS "COLUMN_NAME", c.confrelid::regclass::text AS "REFERENCED_TABLE_NAME" FROM pg_attribute a JOIN pg_constraint c ON (c.conrelid, c.conkey[1]) = (a.attrelid, a.attnum) JOIN pg_class pgc ON pgc.oid = a.attrelid WHERE pgc.relname = ? AND \'\' <> ? AND pgc.relnamespace::regnamespace::text = \'public\' AND c.contype  = \'f\'';
                case 'sqlsrv':
