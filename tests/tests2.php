...
class API
{
	...
	public function __construct($test, $dbConfig)
	{
		$this->test = $test;
	...
	private function action($method,$url,$data='')
	{
		...
		$this->api = new PHP_CRUD_API(array(
				'dbengine'=>$this->dbConfig['dbengine'],
...
class PHP_CRUD_API_Test extends PHPUnit_Framework_TestCase
{
    ...
    {
        if (PHP_CRUD_API_Config::$config) {
            static::$dbConnections = PHP_CRUD_API_Config::$config;
        } else {
	...
	private function setUpDatabase($connection = [])
	{
    ...
    {
        foreach (static::$dbConnections as $connection) {
            if (static::$connection != $connection) {
	...
	{
		$test = new API($this, static::$connection);
		$test->get('/posts');
